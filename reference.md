...
### 図を保存...
現在表示されている平面図・縦断面図を*hoge*.*xxx*ファイルに出力します。
...
```
距離程 [m], x座標位置 [m], y座標位置 [m], z座標位置 [m], x軸に対する方位角 [radian], 曲線半径 [m], 勾配 [‰], 緩和曲線関数(0: sin半波長逓減, 1: 直線逓減), カント [m], カント回転中心 [m], 軌間 [m]
```
...
~~* 変数に多バイト文字を使っている
    * 変数名に利用できる文字は、半角のA-Z, a-z, 0-9, _のみ~~ (Ver. 1.1.7で全てのUnicode文字に対応)
* 変数にマップ要素を代入している
