...
* マップ要素のキーがシングルクォーテーションでくくられていない
    * `Track['key']`とするべきところが`Track[key]`となっている
...
マップファイル、停車場リストファイルを読み込む際のエンコーディング設定は、各ファイルのヘッダーの指定に従います。
ただし、ヘッダーの設定でデコードに失敗した場合、以下の方法でもう一度読み込みを試みます。

* ヘッダーでutf-8指定の場合、Shift-JIS (より正確には[CP932](https://ja.wikipedia.org/wiki/Microsoftコードページ932))でのデコードを試みる
* ヘッダーでShift-JIS指定の場合、utf-8でのデコードを試みる
...
また、実行中に何らかのエラーが発生した場合にpython標準のデバッガ[pdb](https://docs.python.org/ja/3/library/pdb.html)が起動します。
